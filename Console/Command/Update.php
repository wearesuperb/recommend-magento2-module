...
class Update extends Command
{
    ...
    {
        $this->setName('recommend:update');
        $this->setDescription('Update data with Recommend_2');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($websites as $_website) {
            ...
            if (isset($categoriesBatch[$_website->getCode()])) {
                $this->_helperApi->syncCategories($categoriesBatch[$_website->getCode()], $_website->getCode(), null);
            }

            if (isset($productBatch[$_website->getCode()])) {
                if (isset($productBatch[$_website->getCode()]['products'])) {
                    $this->_helperApi->syncProducts($productBatch[$_website->getCode()]['products'], $_website->getCode(), null);
                }
                if (isset($productBatch[$_website->getCode()]['variants'])) {
                    $this->_helperApi->syncVariants($productBatch[$_website->getCode()]['variants'], $_website->getCode(), null);
                }
            }
        }
    }
